...
def solution(lines: list[str]):
    ...
    # Make Connections
    connections = 0
    while connections < CONNECTIONS_TO_MAKE:
    ...
    # Get All Circuits & Sizes
    root_to_size = {}
    for p in points:
        root = circuits.find(p)
        root_to_size[root] = circuits.size[root]

    result = reduce(lambda x, y: x*y, sorted(root_to_size.values(), reverse=True)[:3], 1)
    print(result)
